...
resol_model_file = "./models/RRDB_ESRGAN_x4.pth"
...
resol_model = arch.RRDBNet(3, 3, 64, 23, gc=32)
resol_model.load_state_dict(torch.load(resol_model_file), strict=True)
resol_model.eval()
resol_model = resol_model.to(device)
...
def main(argv):
    ...
    #        cv2.imshow("camera window", image_np)
            ...
            #            image_trm = image_np[y : y + h, x : x + w]
                    ...
                    image_trm = image_trm * 1.0 / 255
                    img_resol = torch.from_numpy(np.transpose(image_trm[:, :, [2, 1, 0]], (2, 0, 1))).float()
                    img_LR = img_resol.unsqueeze(0)
                    img_LR = img_LR.to(device)

                    with torch.no_grad():
                        output_resol = resol_model(img_LR).data.squeeze().float().cpu().clamp_(0, 1).numpy()
                    output_resol = np.transpose(output_resol[[2, 1, 0], :, :], (1, 2, 0))
                    output_resol = (output_resol * 255.0).round()

#                    image_trm_resize = cv2.resize(image_trm, dsize=(150,200))
                    image_trm_resize = cv2.resize(output_resol, dsize=(150,200))

#                    cv2.imwrite(image_out+"/trimed_"+str(i)+"_"+os.path.basename(file),image_trm_resize)
                    cv2.imwrite(image_out+"/trimed_"+str(i)+"_s-resol_"+os.path.basename(file),image_trm_resize)
